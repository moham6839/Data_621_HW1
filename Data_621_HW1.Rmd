...
The `TARGET_WINS` column follows a normal distribution. This will be important when deciding which transformation method to use. Since `TARGET_WINS` will be our dependent variable, using a Box-Cox transformation may not be the best method to use since our dependent variable follows a normal distribution 
...
```{r}
set.seed(123)
m2_log2 <- lm(TARGET_WINS ~ TEAM_BATTING_BB + TEAM_BASERUN_SB + TEAM_BASERUN_CS + TEAM_PITCHING_HR + TEAM_PITCHING_H + TEAM_PITCHING_BB + TEAM_FIELDING_DP, data = mlb_train_features2)
summary(m2_log2)
```
...
```{r}
ggplot(data = m3_train, aes(x = m3$residuals)) +
    geom_histogram(bins = 10, fill = 'steelblue', color = 'black') +
    labs(title = 'Histogram of Residuals', x = 'Residuals', y = 'Frequency')
```
...
```{r}
ggplot(data = m4_log, aes(x = m4$residuals)) +
    geom_histogram(bins = 10, fill = 'steelblue', color = 'black') +
    labs(title = 'Histogram of Residuals', x = 'Residuals', y = 'Frequency')
```
