...
author: "Mohamed Hassan-El Serafi, Tony Cheug, Keith Colella, Yina, Qiao, Eddie Xu"
date: "`r Sys.Date()`"
...
```{r}
mlb_train_log <- log(mlb_train_imp)
```
...
```{r}
mlb_model <- lm(TARGET_WINS ~., data = mlb_train_imp)
...
**Model Using Train Log Set**


```{r}
mlb_model <- lm(TARGET_WINS ~., data = mlb_train_log)
# Stepwise regression model
step.model <- stepAIC(mlb_model, direction = "both", 
                      trace = FALSE)
summary(step.model)
