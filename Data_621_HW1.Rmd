...
### Model 3 - Using Recursive Feature Elimination
...
```{r}
set.seed(123)
m3 <- lm(TARGET_WINS ~ TEAM_FIELDING_E + TEAM_BATTING_H + TEAM_BASERUN_CS + TEAM_BATTING_BB + TEAM_PITCHING_SO, data = mlb_train_imp)
summary(m3)
```
...
```{r}
ggplot(data = m3, aes(x = m3$residuals)) +
    geom_histogram(bins = 10, fill = 'steelblue', color = 'black') +
    labs(title = 'Histogram of Residuals', x = 'Residuals', y = 'Frequency')
```
...
### Model 4 - Using Recursive Feature Elimination for Log-Transformed Train Set
...
```{r}
mlb_train_log2 <- mlb_train_log %>%
  mutate_if(is.numeric, function(x)ifelse(is.infinite(x), median(mlb_train_log),x))
```
...
```{r}
m4 <- lm(TARGET_WINS ~., data=mlb_train_log)
summary(m4)
```
...
```{r}
m4 <- lm(TARGET_WINS ~ TEAM_FIELDING_E + TEAM_BATTING_H + TEAM_BASERUN_SB + TEAM_PITCHING_SO + TEAM_BATTING_BB, data=mlb_train_log)
summary(m4)
```
...
```{r}
ggplot(data = m4, aes(x = m4$residuals)) +
    geom_histogram(bins = 10, fill = 'steelblue', color = 'black') +
    labs(title = 'Histogram of Residuals', x = 'Residuals', y = 'Frequency')
```
...
```{r}
compare_performance(m1_train_revised, m1_log2, m3, m4)
```
